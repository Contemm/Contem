...
extension AnyTransition {
    ...
    /// 아래에서 위로 슬라이드 + 페이드
    /// 용도: 바텀시트, 알림
    static let slideUpAndFade: AnyTransition = .move(edge: .bottom).combined(with: .opacity)
    
    /// 위에서 아래로 슬라이드 + 페이드
    /// 용도: 드롭다운, 알림 배너
    static let slideDownAndFade: AnyTransition = .move(edge: .top).combined(with: .opacity)
    
    /// 왼쪽으로 슬라이드 + 페이드
    /// 용도: 뒤로가기
    static let slideLeftAndFade: AnyTransition = .move(edge: .trailing).combined(with: .opacity)
    
    /// 오른쪽으로 슬라이드 + 페이드
    /// 용도: 다음 화면
    static let slideRightAndFade: AnyTransition = .move(edge: .leading).combined(with: .opacity)
}
...
extension View {
    /// 기본 애니메이션 적용 (0.3초)
    /// 용도: 일반적인 UI 변화
    func animateDefault<V: Equatable>(value: V) -> some View {
        self.animation(.defaultTransition, value: value)
    }
    
    /// 빠른 애니메이션 적용 (0.2초)
    /// 용도: 탭 선택, 토글
    func animateQuick<V: Equatable>(value: V) -> some View {
        self.animation(.quickTransition, value: value)
    }
    
    /// 느린 애니메이션 적용 (0.5초)
    /// 용도: 페이드 인/아웃
    func animateSlow<V: Equatable>(value: V) -> some View {
